      ...
      <div className="flex flex-wrap gap-2">
        <form action={toggleStoreActiveAction}>
          ...
          <button className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-white transition hover:bg-white/10">
            {store.isActive ? "Pause" : "Enable"}
          </button>
        </form>
        <form action={deleteStoreAction}>
          <input type="hidden" name="storeId" value={store.id} />
          <button className="rounded-full border border-rose-400/25 bg-rose-400/10 px-3 py-2 text-xs font-medium text-rose-100 transition hover:bg-rose-400/20">
            Remove
          </button>
        </form>
      </div>
